...
- `ordenar` (`GET /api/residentes`): `recentes` (padrão no front, `created_at DESC`) | `nome` | `mes_ano`. Em qualquer um dos três, residentes com status encerrado sem continuidade (`STATUS_ENCERRADOS = ('Cancelado', 'Indeferido', 'Desistente', 'Nao veio')`) são forçados pro fim da lista (`PRIORIDADE_ENCERRADO` como primeira chave do `ORDER BY`) — ficam nas últimas páginas em vez de aparecer misturados com casos ativos. `'Concluído'` fica de fora de propósito (é desfecho de sucesso, não precisa ser escondido).
- `mes_inscricao` (`GET /api/residentes` e `/api/residentes/exportar-csv`, filtro `#f-mes-inscricao` em `residentes.html`): filtra pelo mês/ano em que o pedido foi **de fato enviado** (`data_inscricao`), diferente de `mes_ano` (mês do estágio **desejado** — outro filtro, `#f-mes`, que pode apontar pra um mês passado ou futuro sem relação com quando o formulário foi preenchido). `data_inscricao` é texto livre de import em dois formatos (`DD/MM/AAAA HH:MM` ou ISO); `SQL_MES_INSCRICAO` em `app.py` extrai `'AAAA-MM'` via `CASE` pros dois formatos — mesmo padrão usado em `ai.py` para o snapshot da IA (ver seção Assistente IA).
...
- **Etapa 3 (`area_medica`) manda para o chefe, não para o aluno**: `pipelineAcaoAtual.telefoneDestino` começa como o telefone do próprio residente (`a.telefone`, default de `abrirModalPipelineAcao`); a etapa 3 precisa sobrescrever isso com o celular do contato da área médica, senão o WhatsApp abre com o número errado. Por isso essa etapa também renderiza um `<select id="pa-campo-contato-am">` em `#pa-extra-campos` (mesmo padrão do `am-select` do botão avulso "Falar com Área Médica"), pré-selecionado via `melhorMatchAreaMedica`, sempre conferível/trocável pelo usuário; `recalcularMensagemPipeline` atualiza `telefoneDestino` e a mensagem juntos a cada troca no select.
...
- **Histórico multi-ano e multi-mês**: o snapshot inclui `por_ano_do_pedido`/`por_ano_do_pedido_e_status`/`por_ano_do_pedido_e_especialidade` e os equivalentes por mês (`por_mes_do_pedido`/`por_mes_do_pedido_e_status`, granularidade `'AAAA-MM'`) — contagens agrupadas pela data do campo `data_inscricao` (quando o pedido foi de fato enviado), **não** `mes_ano` (mês do estágio desejado, que pode ser passado/futuro e não indica a data do pedido). São o que permite a IA responder perguntas tipo "quantos pedidos entraram em 2025" ou "liste os meses" **sem precisar contar registro por registro** em `recentes` — importante porque o modelo padrão (`tencent/hy3:free`) é de raciocínio e pode gastar todo o `max_tokens` "pensando" sem nunca escrever a resposta final se for forçado a somar manualmente; os campos agregados prontos evitam esse erro (`api_ai_chat` também usa `max_tokens=4096`, acima do default de `chamar_openrouter`, como margem extra).
- **Acesso amplo, somente leitura, sem PII**: por decisão explícita do usuário, a IA tem visão ampla dos dados de Residentes — `por_especialidade` (até 40), `por_status_pagamento` (todos), `recentes` (até 80 registros, não mais 15) e `pipeline_atendimento` (mesma agregação de `/api/pipeline/dashboard`: pendentes por etapa, críticos >14d, ações concluídas). O único limite que permanece — e é definitivo — é **nunca CPF, e-mail ou telefone**; esses campos simplesmente não entram em nenhuma query de `montar_snapshot()`. Snapshot completo hoje gira em torno de ~6-7k tokens.
- **Endpoints** (`@login_required`): `GET /api/ai/status` → `{enabled}`; `POST /api/ai/chat` (body `{messages:[...]}`, histórico limitado a 12, resposta de uma vez só); `POST /api/ai/chat/stream` (mesmo body, mas devolve a resposta aos pedaços via `Response(stream_with_context(...), mimetype='text/plain')` — é o que o painel de chat em `ai.js` usa de fato, `enviar()` lê com `response.body.getReader()` e vai atualizando a bolha da mensagem); `GET /api/ai/insights` (resumo executivo, continua não-streaming). Erros viram `AIError` com mensagem amigável (HTTP 502 nas rotas não-streaming; no streaming, se falhar no meio, o erro é escrito como texto dentro do próprio stream, já que o `Response` 200 já começou).
- **Sem teto artificial de tokens**: `MAX_TOKENS_PADRAO=16000` em `ai.py` (modelo padrão é free-tier, sem custo por token — o teto é só rede de segurança, não economia). `stream_openrouter()` só repassa `delta.content`, nunca `delta.reasoning` (o raciocínio interno do modelo não deve aparecer pro usuário). A maior parte da latência de um modelo de raciocínio como o `hy3:free` é gasta "pensando" antes do primeiro pedaço de conteúdo chegar (observado: ~17s de "silêncio" antes do stream começar, numa resposta de 3 frases) — streaming não elimina essa espera, só evita esperar tudo de uma vez e mostra o texto sendo "digitado" assim que a IA começa a escrever.
