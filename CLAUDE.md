...
- `ordenar` (`GET /api/residentes`): `recentes` (padrão no front, `created_at DESC`) | `nome` | `mes_ano`. Em qualquer um dos três, residentes com `status='Cancelado'` são forçados pro fim da lista (`PRIORIDADE_CANCELADO` como primeira chave do `ORDER BY`) — assim ficam nas últimas páginas em vez de aparecer misturados com casos ativos.
